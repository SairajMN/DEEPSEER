...
function toBase64Json(value2) {
  const bodyBytes = new TextEncoder().encode(JSON.stringify(value2));
  return Buffer.from(bodyBytes).toString("base64");
}
var requestPriceData = (config) => (sendRequester) => {
  const req = {
  ...
  const unixTimestamp = typeof rawTimestamp === "number" && Number.isFinite(rawTimestamp) ? Math.floor(rawTimestamp) : Math.floor(Date.now() / 1000);
...
function fetchExternalData(runtime2) {
  const httpClient = new cre.capabilities.HTTPClient;
  return httpClient.sendRequest(runtime2, requestPriceData(runtime2.config), consensusIdenticalAggregation())(runtime2.config).result();
}
function buildRiskRequestBody(marketAddress, externalData) {
  return toBase64Json({
...
function submitSettlementReport(runtime2, marketAddress, externalData, risk) {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.evm.chainSelectorName,
    isTestnet: true
  });
  if (!network248) {
    throw new Error(`Unknown network: ${runtime2.config.evm.chainSelectorName}`);
  }
  const evmClient = new cre.capabilities.EVMClient(network248.chainSelector.selector);
  const reportData = encodeAbiParameters(parseAbiParameters("address market, int256 externalPrice, uint64 externalPriceTimestamp, uint16 confidenceScore, bool anomalyFlag, uint16 sourceConsensus, bytes32 evidenceHash"), [
    marketAddress,
  ...
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.evm.settlementEngineAddress,
    report: signedReport,
    gasConfig: {
      gasLimit: runtime2.config.evm.gasLimit
    }
  }).result();
  return bytesToHex(writeResult.txHash ?? new Uint8Array(32));
}
var httpUrlSchema = exports_external.string().regex(/^https?:\/\/\S+$/u, "must be a valid http(s) URL");
var evmSchema = exports_external.object({
  chainSelectorName: exports_external.string().min(1),
  settlementEngineAddress: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/u),
  gasLimit: exports_external.string().regex(/^\d+$/u)
});
var externalDataSchema = exports_external.object({
  priceApiUrl: httpUrlSchema,
  assetPath: exports_external.string().min(1),
  timestampPath: exports_external.string().min(1),
  priceScale: exports_external.number().int().positive()
});
var riskApiSchema = exports_external.object({
  url: httpUrlSchema,
...
var riskResponseSchema = exports_external.object({
  confidence_score: exports_external.number().int().min(0).max(1e4),
  anomaly_flag: exports_external.boolean(),
  source_consensus: exports_external.number().int().min(0).max(1e4),
  evidence_hash: exports_external.string().regex(/^0x[a-fA-F0-9]{64}$/u)
});
var requestRiskScore = (config, marketAddress, externalData, apiKey) => (sendRequester) => {
  const req = {
...
function fetchRiskScore(runtime2, marketAddress, externalData) {
  const apiKey = runtime2.getSecret({ id: "RISK_API_KEY" }).result().value;
  const httpClient = new cre.capabilities.HTTPClient;
  return httpClient.sendRequest(runtime2, requestRiskScore(runtime2.config, marketAddress, externalData, apiKey), consensusIdenticalAggregation())(runtime2.config).result();
}
var eventAbi = parseAbi([
  "event ResolutionWindowOpenedByEngine(address indexed market)"
]);
var triggerSignature = "ResolutionWindowOpenedByEngine(address)";
function handleResolutionTrigger(runtime2, log) {
  ...
  runtime2.log(`Resolution trigger received for market ${marketAddress}`);
  const externalData = fetchExternalData(runtime2);
  runtime2.log(`External data fetched. price=${externalData.externalPrice.toString()} timestamp=${externalData.externalTimestamp.toString()}`);
  const riskResponse = fetchRiskScore(runtime2, marketAddress, externalData);
  runtime2.log(`Risk score received. confidence=${riskResponse.confidence_score} anomaly=${riskResponse.anomaly_flag} consensus=${riskResponse.source_consensus}`);
  const txHash = submitSettlementReport(runtime2, marketAddress, externalData, riskResponse);
  runtime2.log(`Settlement report submitted: ${txHash}`);
  return txHash;
}
